...
test(
  'context does not block active feature execution for docs/AGENTS.md',
  async () => {
    await withTempDir('lsk-context-managed-docs-agents-', async (dir) => {
      const initResult = await runCli(dir, [
        'init',
        '--non-interactive',
        '--name',
      ...
      const payload = JSON.parse(result.stdout.trim());
      assert.equal(payload.status, 'single_matched');
      assert.notEqual(payload.primaryActionCategory, 'docs_normalize');
      assert.notEqual(payload.matchedFeature.currentSubstateId, 'docs_unmanaged_normalize');
      assert.equal(
        (payload.matchedFeature.warnings || []).some((warning) =>
          /unmanaged docs/i.test(String(warning))
        ),
