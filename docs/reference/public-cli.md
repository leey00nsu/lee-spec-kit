...
npx lee-spec-kit knowledge ci --auto-merge --json
...
Auto-merge is opt-in with `--auto-merge`. It adds a secretless `Knowledge PR safety` check on pull requests and enables GitHub auto-merge only for a complete, current Knowledge PR. Before enabling it in the repository, turn on **Allow auto-merge** and require that check on the base branch with an up-to-date branch requirement; decide separately whether human review is required. Other pull requests pass through the Knowledge-specific check. The project-owned workflow can customize its schedule and model secrets after scaffolding; `knowledge ci` preserves a differing workflow instead of overwriting it.
