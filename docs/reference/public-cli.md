...
Publish the optional OpenWiki onboarding layer from integrated code. Local completion calls `knowledge publish <feature>` after post-merge verification and before cleanup. GitHub uses the integration-branch push workflow written by `knowledge ci`; configure its provider secret and commit the workflow before use. CI setup is explicit and never pushes files or provisions credentials. PR events do not generate the public Wiki. Legacy `sync` and `audit` operate on in-place output for compatibility, not as Feature gates.
...
When `experimental.openwiki` is `true`, Feature review no longer requires generated Wiki pages or a Knowledge commit. Curated documentation checks still run before Feature review. Local integration returns a `knowledge_sync` action invoking `knowledge publish` after the merged code passes verification. With completion strategy `none`, there is no automatic publication because no integration is performed. GitHub publishing uses CI after a push to the configured base branch.

```bash
npx lee-spec-kit knowledge publish F001-alpha --json
...
Publication creates a detached worktree for the exact integration commit under the shared Git runtime directory. Only validated `openwiki/`, the receipt, and a publication manifest are copied to a unique artifact directory. The source checkout, index, branches, and commits are unchanged. The returned `artifactPath` is the browsing/upload root. A repository-wide publication lock serializes generators. The authoritative pointer is a blob at `refs/lee-spec-kit/knowledge/publication`; a Git ref transaction verifies the exact base tip and updates that pointer atomically, including against concurrent merge/ref writers. A changed base ref rejects publication, even if it changes during temporary-worktree cleanup. CI cancels superseded runs and uploads immutable SHA/run-attempt artifacts, including hidden claim/manifest files; it does not push a shared latest branch. A failed run keeps the last good artifact and the successful code merge. `knowledge status` reports the failure and retained diagnostic worktree; retry `knowledge publish` to start a fresh isolated run. Failed runs are retained locally for manual inspection and consume disk until removed with Git worktree tooling. Successful publication removes its temporary worktree.
