...
Publish the optional OpenWiki onboarding layer from integrated code. Knowledge freshness is observational and never blocks Feature integration or cleanup. GitHub uses the scheduled and manually dispatched workflow written by `knowledge ci`; configure its provider secret and commit the workflow before use. CI setup is explicit and does not provision credentials. It updates one reusable Knowledge-only pull request. Legacy `sync` and `audit` operate on in-place output for compatibility, not as Feature gates.
...
When `experimental.openwiki` is `true`, Feature review no longer requires generated Wiki pages or a Knowledge commit. Curated documentation checks still run before Feature review. Knowledge update is independent repository maintenance after integration. `knowledge publish` and its `knowledge update` alias generate the same verified artifact. Local and GitHub Feature completion do not return a Knowledge gate. Scheduled or manually dispatched CI updates one reusable Knowledge pull request.

```bash
npx lee-spec-kit knowledge status --json
npx lee-spec-kit knowledge ci --json
# CI checks out and fetches the integration branch; no Feature selector is needed:
npx lee-spec-kit knowledge publish --ci --base-branch main --lang en --json
```

Publication creates a detached worktree for the exact integration commit under the shared Git runtime directory. Only validated `openwiki/`, the receipt, and a publication manifest are copied to a unique artifact directory. The source checkout, index, branches, and commits are unchanged until `knowledge apply` creates a verified Knowledge-only commit. A repository-wide publication lock serializes generators. The authoritative local pointer is a Git ref transaction bound to the exact base tip. Scheduled CI serializes runs, reuses an open Knowledge PR when its verified receipt already covers the base revision, and force-with-lease updates that one branch only for a new revision. A failed run keeps the code integration and last good Knowledge; sanitized result/status diagnostics and a partial-output file inventory are uploaded without raw process output or the OpenWiki queue. A later scheduled run for the same exact revision stops when either that failure artifact or a failed, cancelled, or timed-out workflow run exists; inspect the workflow result and use manual dispatch for an explicit retry. Local failed runs retain their diagnostic worktree for inspection. `knowledge status` validates tracked output and its receipt directly, so a fresh clone does not require the machine-local publication cache.

The generated GitHub.com workflow uses Node.js 22, OpenWiki 0.5.2, the generating toolkit's exact version, and an `OPENAI_API_KEY` repository secret. It installs project dependencies before applying the verified commit so repository commit hooks can run. Change the provider configuration explicitly if using another provider. GitHub-hosted runners still impose their platform maximum runtime. A platform termination can prevent the final diagnostic upload, but its workflow conclusion prevents an automatic repeat for the same revision. Artifacts have GitHub retention limits and are not a permanent hosted Wiki; a deployment destination can consume a successful SHA-bound artifact separately. Standalone projects scaffold into the selected project repository; CI can publish without the external docs repository using explicit base/language options. External curated docs are not included in that source snapshot. When a checked-out config disables OpenWiki, CI skips generation and upload. A standalone CI checkout without its external docs config is explicitly opted in by the scaffolded command; disable or remove that workflow to stop publication.
...
Read-only visualization is allowed directly and does not replace the managed Knowledge CI. The Codex hook limits this exception to `./openwiki`, an optional numeric `--port`, and `--no-open`. `visualize --export` writes a static site and remains blocked in the automated path; run it manually only after reviewing its destination and commit policy.
