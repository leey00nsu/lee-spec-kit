...
`knowledge migrate` defaults to a zero-write dry-run. `--apply` adds only a policy-cutover marker to approved, terminal, fully committed legacy Plans that lack the entire Curated Documentation Impact section. It never invents `NONE` decisions, never runs OpenWiki, and leaves active, dirty, malformed, or partially assessed Features for manual review.

The adapter also maintains a final lee-spec-kit block in `.openwikiignore` for common environment, key, certificate, credential, and secret paths; user-authored ignore rules before that block are preserved. lee-spec-kit never installs OpenWiki or provider credentials.
