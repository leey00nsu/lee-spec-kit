...
Plan, task, and Feature review actions also return `reviewRound` and `maxReviewRounds`. Record `reviewRound` in the matching review metadata. `workflow.agentReview.maxRounds` counts automatic finding-remediation passes rather than the initial review and defaults to `1`. With `1`, findings from review round 1 are remediated once and round 2 reviews the result; another `changes_requested` decision then returns `review_escalation`. The escalation sets `approvalRequired: true` and `implementationAllowed: false` and exposes exact user action options. Do not remediate or delegate another review until the user chooses retry and `maxRounds` is increased.
