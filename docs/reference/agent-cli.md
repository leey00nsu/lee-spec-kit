...
Plan, task, and Feature review actions also return `reviewRound` and `maxReviewRounds`. Record `reviewRound` in the matching review metadata. `workflow.agentReview.maxRounds` counts automatic finding-remediation passes rather than the initial review and defaults to `1`. With `1`, findings from review round 1 are remediated once and round 2 reviews the result. If round 2 still returns `changes_requested`, preserve those findings as residual risks and automatically complete the review gate without requesting a user review-approval token. `blocked` decisions never auto-complete.
