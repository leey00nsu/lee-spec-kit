...
With OpenWiki enabled, keep Feature review focused on code and curated docs. After verified local integration, follow `knowledge_sync` by running the returned `knowledge publish` command; GitHub uses `knowledge ci` to scaffold publication after base-branch pushes. Generated pages and receipts are separate revision-bound artifacts, not Feature commits or required review documents.
