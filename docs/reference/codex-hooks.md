...
- When `experimental.openwiki=true`, direct repository-generating OpenWiki commands are blocked so they cannot bypass the scheduled/manual CI's managed generation, receipt, and freshness validation. Exact simple `--help`, connector `auth`, and read-only `visualize` forms limited to `./openwiki` remain available. `visualize --export`, paths outside `./openwiki`, shell substitution, wrappers, unknown execution forms, `--init`, `code`, and other repository-mutating surfaces fail closed. OpenWiki 0.5.x couples ChatGPT provider login to `code --init`, so that one-time upstream setup must be run manually in a trusted terminal and followed by `knowledge doctor` and the Knowledge CI workflow; `/provider` alone does not log in. Missing or `false` adds no OpenWiki-specific hook behavior.
