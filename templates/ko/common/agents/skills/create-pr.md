...
## Pre-PR 리뷰 폴백(`builtin-checklist`)

설정된 리뷰 스킬을 실행할 수 없을 때 아래 기준으로 진행합니다.

1. `spec.md` / `plan.md` / `tasks.md` 기준으로 변경 범위 정합성을 확인하고 회귀/예외/보안 리스크를 점검합니다.
2. 관련 테스트/검증 명령을 실행합니다. 실행하지 못했다면 사유를 명시합니다.
3. `tasks.md`의 `PR 전 리뷰 Findings`를 `major=<n>, minor=<n>` 형식으로 최신화합니다.
4. `PR 전 리뷰 Evidence`에 실제 근거(리뷰 링크, 로그, 문서 경로)를 기록합니다.
5. 체크리스트 완료 후에만 `PR 전 리뷰`를 `Done`으로 변경합니다.
