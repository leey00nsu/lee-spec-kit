...
- The implementation worker executes the assigned task directly. It must not run `workflow-stage`, delegate again, change task state, commit, request approval, or perform remote/destructive actions. It may edit project code, run task-scoped checks, and edit only the exact curated-document paths in `workerContract.allowedWritePaths` when `workerContract.editDocs === true`; Feature docs remain forbidden because `editFeatureDocs` is false.
