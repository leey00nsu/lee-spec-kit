...
function createFeatureReviewDelegationContext(
  ...
  const requiredDocuments = [
    ...
    createDelegationDocument(
      paths.decisionsPath,
      'Review recorded decisions, trade-offs, and residual risks. Check Curated Documentation Impact evidence, including NONE. README protection takes precedence: modify an existing README (root, nested, or localized) only on an explicit user request to edit it; generic implementation or documentation-sync requests and agent-authored Plans do not authorize edits. Without that request, a README discrepancy may remain deferred with its path, evidence, and deferral reason recorded in decisions.md. Assess other documentation impacts independently; a surface with only this deferred README impact may use NONE referencing that record, without claiming the discrepancy is absent or resolved. This exception requires no separate follow-up task/Feature/issue or README edit approval; do not block solely because the README remains unchanged. Every other discovered documentation discrepancy needs a completed correction or a real follow-up task/Feature/issue reference with document paths, evidence, the unresolved question, and the deferral reason. A residual-risk note alone is not follow-up tracking. Report missing disposition as a finding; never infer product intent from code or generated Knowledge.'
    ),
  ];
