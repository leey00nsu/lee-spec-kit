...
12. When docs are synced to code, run \`npx lee-spec-kit workflow-audit --json\` and copy its exact \`expectedWorkflowSyncMarker\` into one active feature doc (prefer \`tasks.md\` or \`decisions.md\`): replace an existing marker or remove duplicates instead of appending another marker, so the marker is bound to the current code-content fingerprint
   ...
   - When \`experimental.openwiki === true\`, publish Knowledge only after integration: local workflows return \`knowledge_sync\` with \`knowledge publish\` after verified merge and before cleanup; GitHub uses the integration-branch CI scaffolded by \`knowledge ci\`. Generated Wiki and receipts are revision-bound artifacts outside Feature commits/reviews. Failed generation preserves the merge and last good artifact; inspect \`knowledge status\` and retry
