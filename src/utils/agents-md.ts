...
12. When docs are synced to code, keep exactly one explicit marker like \`<!-- lee-spec-kit:workflow-sync 2026-04-16T12:34:56.789Z -->\` in a single active feature doc (prefer \`tasks.md\` or \`decisions.md\`): replace an existing marker timestamp or remove duplicates instead of appending another marker, so \`workflow-audit\` can prove the sync happened after the latest code change
   ...
   - Keep authority layered: Feature SDD docs are normative for requirements and decisions, curated project-wide docs describe the human-owned current state, and OpenWiki is derived onboarding evidence that must be verified against tracked sources
   - When \`experimental.openwiki === true\`, follow the returned \`knowledge_setup\`, \`knowledge_sync\`, and \`knowledge_commit\` actions after task checkpoints and before Feature review. Run OpenWiki only through \`lee-spec-kit knowledge sync\`, commit only the verified Knowledge surface with the exact returned subject, and include the generated index plus receipt in the required Feature review
